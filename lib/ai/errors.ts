...
export function normalizeAIError(error: unknown) {
  ...
  if (error instanceof AIProviderHTTPError) { if (error.status === 401) return { message: "302.AI authentication failed. Please check AI_302_API_KEY.", code: error.code, status: error.status }; if (error.status === 403) return { message: `302.AI denied this endpoint or model. The API key may be valid, but the selected model, voice, or account permission is unavailable. ${safeProviderDetail(error.message)}`, code: error.code, status: error.status }; if (error.status === 402 || error.status === 429) return { message: "302.AI request was limited. Please check your balance, quota, or rate limit.", code: error.code, status: error.status }; if (error.status >= 500) { const detail = safeProviderDetail(error.message); return { message: detail && detail !== "Unknown response" ? `302.AI service returned ${error.status}: ${detail}` : `302.AI service returned ${error.status}. Please retry later or use the configured default model.`, code: error.code, status: error.status }; } return { message: safeProviderDetail(error.message), code: error.code, status: error.status }; }
