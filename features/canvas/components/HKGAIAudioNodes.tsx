...
const builtInVoices = ["Mandarin_治愈女声", "Mandarin_沉稳男声", "Cantonese_暖心师奶", "English_元气女声"];
  ...
  return (
    <>
      ...
      <div className={`${panel} ${selected ? "translate-y-0 opacity-100 pointer-events-auto" : "-translate-y-4 opacity-0 pointer-events-none"}`}>
        ...
        <div className="mt-4 flex items-center justify-between"><p className="text-[10px] text-[#676f7b] dark:text-slate-400">段落之间用分号分隔；接口暂未提供时长参数。</p><button className={primaryButton} disabled={isRunning || !data.musicTags?.trim() || !data.prompt?.trim()} onClick={() => void runNode(id)}>{isRunning ? "Generating..." : "Generate music"}</button></div>
      </div>
    </>
  );
}
...
export function HKGAITTSNodeLayout({ id, data, selected, runNode }: { id: string; data: CanvasNodeData; selected: boolean; runNode(id: string): Promise<void> }) {
  ...
  const isRunning = data.status === "running" || data.status === "waiting";
  const audioUrl = audioUrlFrom(data);
  const usingBuiltIn = builtInVoices.includes(data.voice || "");
  ...
  const uploadVoice = async () => {
    if (!file) return updateNodeData(id, { error: "请选择 1–30 秒、10 MB 以内的参考音频。" });
    setUploading(true);
  ...
  return (
    <>
      <NodeShell id={id} data={data} selected={selected} title="HKGAI TTS" subtitle="Voice synthesis">
        <div className="mt-3 flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${data.voice ? "bg-emerald-500" : "bg-slate-300"}`} /><p className="truncate text-sm font-bold text-[#030303] dark:text-slate-100">{data.voice || "No voice selected"}</p></div>
      ...
      <div className={`${panel} ${selected ? "translate-y-0 opacity-100 pointer-events-auto" : "-translate-y-4 opacity-0 pointer-events-none"}`}>
        <label><span className={labelClass}>Text</span><textarea className={`${inputClass} min-h-24 resize-y`} value={data.ttsText || ""} placeholder="留空时使用连接的上游文本" onChange={(event) => updateNodeData(id, { ttsText: event.target.value })} /></label>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <label><span className={labelClass}>Built-in voice</span><select className={inputClass} value={usingBuiltIn ? data.voice : ""} onChange={(event) => updateNodeData(id, { voice: event.target.value })}><option value="">Custom voice_id</option>{builtInVoices.map((voice) => <option key={voice} value={voice}>{voice}</option>)}</select></label>
        ...
        <div className="mt-3 grid grid-cols-[1fr_150px] gap-3">
          <label><span className={labelClass}>Instructions</span><input className={inputClass} value={data.ttsInstructions || ""} placeholder="温柔、自然、像在聊天" onChange={(event) => updateNodeData(id, { ttsInstructions: event.target.value })} /></label>
        ...
        <div className="mt-4 rounded-2xl border border-[#e7eaf0] bg-[#fafafa] p-3 dark:border-slate-700 dark:bg-[#0c1622]">
          ...
          <label className="mt-3 flex items-start gap-2 text-[11px] leading-4 text-[#404040] dark:text-slate-200"><input className="mt-0.5" type="checkbox" checked={data.consentConfirmed === true} onChange={(event) => updateNodeData(id, { consentConfirmed: event.target.checked })} /><span>我确认已获得声音所有者的明确授权，并有权用这段录音创建合成声音。</span></label>
