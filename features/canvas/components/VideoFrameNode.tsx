...
export function VideoFrameNode({ id, data, selected }: { id: string; data: CanvasNodeData; selected: boolean }) {
  ...
  const sourceLabel = source?.data.title || "No video connected";
  ...
  return (
    <>
      <div className={`relative flex h-[330px] w-[380px] flex-col rounded-[24px] border bg-white shadow-sm transition-colors dark:bg-[#101c29] ${selected ? "z-50 border-[#030303] dark:border-cyan-400" : "border-[#e7eaf0] dark:border-slate-700"}`}>
        ...
        <div className="absolute -top-8 left-1 text-[20px] font-bold tracking-tight text-[#030303] dark:text-slate-100">{data.title || "Video* Extract Frames"}</div>

        <div className="absolute -left-[78px] top-1/2 flex -translate-y-1/2 items-center gap-2">
          <span className={`text-[12px] font-bold text-[#5f18c8] transition-opacity dark:text-violet-300 ${selected ? "opacity-100" : "opacity-0"}`}>Video</span>
          <div className={`relative h-[18px] w-[18px] rounded-full border-[2.5px] border-[#7322e3] ${source ? "bg-[#7322e3]" : "bg-white dark:bg-[#101c29]"}`}>
            <Handle type="target" id="video" position={Position.Left} aria-label="Video input" className="!absolute !inset-0 !m-auto !h-[28px] !w-[28px] !transform-none !border-0 !bg-transparent opacity-0" />
          </div>
        </div>

        <Handle type="source" id="image" position={Position.Right} aria-label="Image output" title="Image output" className="!h-3 !w-3 !border-2 !border-white !bg-lime-500 dark:!border-[#101c29]" />
        <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-lime-100 px-2 py-0.5 text-[9px] font-bold text-lime-800 dark:bg-lime-950/80 dark:text-lime-200">Image</span>

        <div className="flex min-h-0 flex-1 flex-col p-5 pb-3">
          <div className="mb-2 flex items-center justify-between gap-3 text-[10px]">
            <span className="truncate font-semibold text-[#676f7b] dark:text-slate-400">Source: {sourceLabel}</span>
            {outputImageUrl && <span className="shrink-0 rounded-full bg-lime-100 px-2 py-0.5 font-bold text-lime-800 dark:bg-lime-950/70 dark:text-lime-200">Output at {Number(data.frameTimestampSeconds || 0).toFixed(2)}s</span>}
          </div>
          ...
          <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-[18px] bg-[#f0f1f3] dark:bg-slate-800">
            {videoUrl ? (
              <>
                ...
                />
                <button type="button" onClick={(event) => { event.stopPropagation(); openPreview(); }} aria-label="Open video preview" title="Open preview" className="nodrag absolute right-2 top-2 z-10 grid h-9 w-9 place-items-center rounded-full bg-black/60 text-white transition hover:bg-black/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">
              ...
              <div className="px-8 text-center">
                ...
                <p className="mt-2 text-[12px] font-semibold text-[#676f7b] dark:text-slate-400">{source ? "The source video has not been generated yet" : "Connect a Video node"}</p>
              </div>
            )}
          </div>

          <div className="mt-2 flex items-center justify-between gap-3">
            <span className="text-[10px] tabular-nums text-[#676f7b] dark:text-slate-400">Current {currentTime.toFixed(2)}s</span>
        ...
        <div className="nodrag grid grid-cols-2 gap-2 border-t border-[#e7eaf0] p-3 dark:border-slate-800">
          <button type="button" onClick={() => void extract("timestamp")} disabled={!videoUrl || isRunning} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#c9ccd1] bg-white px-3 text-[12px] font-bold text-[#30343b] transition hover:border-[#84cc16] hover:text-[#527d0d] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400 disabled:cursor-not-allowed disabled:opacity-45 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
            ...
            {extractingMode === "timestamp" ? "Extracting" : "Current frame"}
          </button>
          <button type="button" onClick={() => void extract("last")} disabled={!videoUrl || isRunning} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-lime-400 px-3 text-[12px] font-bold text-[#142000] transition hover:bg-lime-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-600 disabled:cursor-not-allowed disabled:opacity-45">
            ...
            {extractingMode === "last" ? "Extracting" : "Final frame"}
          </button>
        </div>
