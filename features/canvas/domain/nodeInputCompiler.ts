...
const videoPromptReferences = (prompt: string) => prompt.replace(/@(?:image[_\s-]?|图|素材)?(\d+)/gi, (_, index: string) => `<<<image_${Number(index)}>>>`);
const referencedImageUrlsFrom = (node: CanvasNode, upstream: CanvasNode[]) => {
  ...
  return ids.map((id) => byId.get(id)).filter((item): item is CanvasNode => Boolean(item)).map(imageUrlFrom).filter(Boolean);
};
