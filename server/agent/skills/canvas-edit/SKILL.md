...
- Treat shot count and final deliverable count as separate requirements. If the user asks for one video, create exactly one `video` node even when multiple scene images or keyframes are requested. Create multiple video nodes only when the user explicitly requests separate clips or one clip per shot.
- When the user asks for multiple storyboard/scene images, create one editable `text` prompt and one corresponding `image` node for each requested scene. Give matching text/image pairs the same `params.shotNumber`; keep the scene branches parallel rather than chaining one scene image into the next.
- Reuse selected/reference character images as reference inputs to each relevant generated scene image. Each scene text must describe only its own shot while preserving the referenced subject identity and continuity.
- If the user asks to combine several storyboard images into one generated video, create one image-capable `video` node whose `dependsOn` contains every relevant generated image. Do not substitute character turnaround sheets, a scene nine-grid, multiple per-shot videos, or a `videoEdit` node unless the user requests those deliverables.
- Preserve explicit quantities and assembly language from the request: N scene images means N editable scene-image branches; one final video means one final video-generation node; separate clips means separate video nodes; concatenating existing clips means `videoEdit`.
- When creating a new generated-video workflow branch, create one `output` node that depends on the final `video`, `videoEdit`, or `motion` node.
...
- Put the FFmpeg edit plan in `params.editPlan` or `dataPatch.editPlan` as a JSON object, never a natural-language sentence. Keep the user's original instruction in the operation reason or node `prompt`. Supported shape:
...
- `clips[]` order is the final playback order. Supported per-clip controls are `start`, `end`/`duration`, `muted`, `volume` (0-3), `fadeIn`/`fadeOut` (0-10 seconds), `speed` (0.5-2), `rotate` (0/90/180/270), and `fit` (`contain`, `cover`, `stretch`). Clip fades go through black with matching audio fades; do not describe them as overlapping crossfades.
- Clip `start`, `end`, and `duration` are source-local timestamps starting from 0, never positions on the final assembled timeline. Two complete five-second sources must both use `start:0,end:5`; reordering is expressed only by reordering `clips[]`.
- Use the first clip's `fadeIn` and the last clip's `fadeOut` for whole-video entrance/exit fades. Do not duplicate them with root-level `fadeIn`/`fadeOut`.
