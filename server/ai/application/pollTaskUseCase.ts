...
export type PollTaskParams = { type: PollableTaskType; taskId: string; provider?: string; pollUrl?: string; pollAction?: string };

export async function pollTaskUseCase(params: PollTaskParams): Promise<RunNodeResult> {
  const { type, taskId, provider: videoProvider, pollUrl, pollAction } = params;
  ...
  if (type === "video" && videoProvider === "302-sora2") {
    ...
    return { ok: true, provider: "302-sora2", output: await archiveResultMedia(output, { sourceProvider: "302-sora2", sourceTaskId: taskId, mediaTypeHint: "video" }), polling: { intervalMs: 5000 } };
  }
  ...
  if (type === "video" && (videoProvider === "kling" || (!videoProvider && process.env.AI_VIDEO_PROVIDER === "kling"))) {
    ...
    return { ok: true, provider: "kling", output: await archiveResultMedia(output, { sourceProvider: "kling", sourceTaskId: taskId, mediaTypeHint: "video" }), polling: { intervalMs: Number(process.env.KLING_POLL_INTERVAL_MS || 5000) } };
  }
  ...
  return { ok: true, provider: provider.name, output: await archiveResultMedia(output, { sourceProvider: provider.name, sourceTaskId: taskId, mediaTypeHint: type }), polling: { intervalMs: Number(process.env.AI_302_POLL_INTERVAL_MS || 3000) } };
}
