...
export async function runCanvasNode(node: CanvasNode, inputs: unknown[] = []): Promise<NodeOutput> {
  ...
  switch (d.nodeType) {
    ...
    case "script": { const count = clampStoryboardSceneCount(d.numberOfScenes); const value = await textProvider.generateText({ prompt: scriptInstruction(prompt, d.scriptTone || "Cinematic, fictional", count), model: d.model, temperature: 0.5 }); const script = parseScript(value.text, prompt, count); return output("script", script.title, script); }
    case "image": { const value = await imageProvider.generateImage({ prompt, negativePrompt: d.negativePrompt, model: d.model, size: d.size, aspectRatio: d.aspectRatio, resolution: d.resolution, referenceImageUrl: d.referenceImageUrl, referenceImageUrls: d.referenceImageUrl ? [d.referenceImageUrl] : undefined }); return output("image", value.imageUrl ? "Image generated" : value.taskId ? `Image task ${value.taskId} pending` : "Image generation did not return a result", value); }
    ...
    case "storyboard": { const value = await textProvider.generateStoryboard({ storyBrief: prompt, numberOfScenes: clampStoryboardSceneCount(d.targetShotCount ?? d.numberOfScenes), model: d.model }); return output("storyboard", `${Math.min(value.scenes.length, MAX_STORYBOARD_SCENE_COUNT)} scenes created`, value.scenes.slice(0, MAX_STORYBOARD_SCENE_COUNT)); }
