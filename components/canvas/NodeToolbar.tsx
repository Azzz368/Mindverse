      ...
      <div className="space-y-1">
        {nodeTypes.map((type) => (
          <button
            ...
            className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs transition ${ghostType === type ? "bg-[#030303] text-white dark:bg-cyan-600 dark:text-white" : "text-[#1a1a1a] hover:bg-[#f0f1f3] hover:text-[#030303] dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-cyan-200"}`}
          >
            <span className={`grid h-6 w-6 place-items-center rounded text-[#030303] dark:text-cyan-300 ${ghostType === type ? "bg-white/20 text-white dark:text-white" : "bg-[#f0f1f3] dark:bg-slate-800"}`}>
              {symbols[type]}
            </span>
